...
var angle
var do_not_touch_this_fuckass_value = 89.54  #idk why this exists
var jump_velocity = -40000
...
func _physics_process(_delta: float) -> void:
	...
	raycast.rotation = -rotation + do_not_touch_this_fuckass_value
...
func _integrate_forces(_state: PhysicsDirectBodyState2D) -> void:
	...
	if _contacted.size() > 0:
		if _state.get_contact_count() > 1:
			var closest_to_zero:float = 361 #idk what this is but changing this causes problems going up the slopes
			for i in _state.get_contact_count():
				var pos = _state.get_contact_collider_position(i)
				var one_of_angles = rad_to_deg(raycast.get_angle_to(pos))
